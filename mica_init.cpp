...
enum ANALYSIS_TYPE {UNKNOWN_ANALYSIS_TYPE = -1, ALL=0, ILP, ILP_ONE, ITYPES, PPM, MICA_REG, STRIDE, MEMFOOTPRINT, MEMSTACKDIST, MEMREUSEDIST2, CUSTOM, ANA_TYPE_CNT};
const char* analysis_types_str[ANA_TYPE_CNT] = { "all",   "ilp", "ilp_one", "itypes", "ppm", "reg", "stride", "memfootprint", "memstackdist", "memreusedist2", "custom"};
...
enum ANALYSIS_TYPE findAnalysisType(char* s){
	...
	if(strcmp(s, "memreusedist2") == 0){ return MEMREUSEDIST2; }
				...
				switch(findAnalysisType(val)){
					...
					case MEMSTACKDIST:
						*mode = MODE_MEMSTACKDIST;
						...
						break;
					case MEMREUSEDIST2:
						*mode = MODE_MEMREUSEDIST2;
						cerr << "Measuring MEMREUSEDIST2 characteristics..." << endl;
						(*log) << "Measuring MEMREUSEDIST2 characteristics..." << endl;
						break;
					case CUSTOM:
						*mode = MODE_CUSTOM;
